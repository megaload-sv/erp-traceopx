...
class BillingPreparationService
{
    ...
    public function createFromServiceCase(int $serviceCaseId, string $requestedDocumentType = '', string $notes = ''): int
    {
        ...
        try {
            $billingCaseId = $model->insert([
                'uuid'=>$this->uuidV4(),'code'=>$model->nextCode(),'origin_type'=>'quotation','service_case_id'=>$serviceCaseId,
                'work_order_id'=>!empty($source['work_order_id'])?(int)$source['work_order_id']:null,
                'quotation_id'=>(int)$source['accepted_quotation_id'],'customer_id'=>(int)$source['customer_id'],
            ...
            $db->table('service_case_financial_policies')->where('service_case_id',$serviceCaseId)->update(['fiscal_document_type'=>$documentType,'modify_user'=>$this->actor(),'modify_date'=>$now]);
            $db->table('service_cases')->where('id',$serviceCaseId)->update(['billing_status'=>'preparation','next_action_code'=>$preOperationalBilling?'billing.advance_prepare':'billing.review','next_action_label'=>$preOperationalBilling?'Preparar facturación de anticipo':'Revisar preparación de facturación','modify_user'=>$this->actor(),'modify_date'=>$now]);
            $db->table('service_case_events')->insert(['service_case_id'=>$serviceCaseId,'event_code'=>'billing.preparation_created','title'=>'Preparación de facturación creada','description'=>self::DOCUMENT_TYPES[$documentType].' · Total base $'.number_format($total,2).($preOperationalBilling?' · Preparación anticipada por política financiera.':''),'entity_type'=>'billing_case','entity_id'=>(int)$billingCaseId,'occurred_at'=>$now,'entry_user'=>$this->actor(),'entry_date'=>$now]);
            $db->transCommit();
    ...
    private function createSchedule(int $billingCaseId, float $total, bool $requiresAdvance, float $advancePercentage): void
    {
        ...
        if($requiresAdvance&&$advancePercentage>0&&$advancePercentage<100){$advance=round($total*($advancePercentage/100),2);$rows=[['sequence'=>1,'concept'=>'Anticipo comercial','installment_type'=>'advance','percentage'=>$advancePercentage,'amount'=>$advance,'trigger_event'=>'commercial_acceptance'],['sequence'=>2,'concept'=>'Saldo final','installment_type'=>'balance','percentage'=>100-$advancePercentage,'amount'=>round($total-$advance,2),'trigger_event'=>'work_order_completed']];}
        elseif($requiresAdvance&&$advancePercentage>=100){$rows=[['sequence'=>1,'concept'=>'Pago anticipado','installment_type'=>'advance','percentage'=>100,'amount'=>$total,'trigger_event'=>'commercial_acceptance']];}
        else{$rows=[['sequence'=>1,'concept'=>'Pago según condición comercial','installment_type'=>'scheduled','percentage'=>100,'amount'=>$total,'trigger_event'=>'invoice_issue']];}
        foreach($rows as $row){$db->table('billing_payment_schedule')->insert($row+['billing_case_id'=>$billingCaseId,'status'=>'pending','entry_user'=>$this->actor(),'entry_date'=>$now]);}
    }

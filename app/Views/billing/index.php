...
<?php foreach($cases as $row): if($row['billing_case_id']) continue; $policy=$row['financial_policy']??[]; $inheritedType=(string)($row['fiscal_document_type']??''); ?>
...
<div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4"><p class="text-xs font-bold uppercase tracking-wide text-emerald-700">Documento heredado de cotización</p><p class="mt-2 font-bold text-emerald-950"><?= esc($documentTypes[$inheritedType]) ?></p><input type="hidden" name="document_type" value="<?= esc($inheritedType) ?>"></div>
<?php else: ?>
<label><span class="mb-2 block text-sm font-semibold">Documento fiscal objetivo *</span><select name="document_type" required class="w-full rounded-xl border border-slate-300 bg-white px-4 py-3"><option value="">Seleccionar</option><?php foreach($documentTypes as $code=>$label): ?><option value="<?= esc($code) ?>"><?= esc($label) ?></option><?php endforeach ?></select><span class="mt-1 block text-xs text-slate-500">La cotización no tenía documento fiscal definido; Facturación debe completarlo.</span></label>
<?php endif ?>

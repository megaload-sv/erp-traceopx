...
<div class="mb-7 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
    <div><p class="text-xs font-semibold uppercase tracking-[.2em] text-cyan-600">DTE Engine</p><h2 class="mt-2 text-3xl font-bold text-slate-950">Emisor y parámetros de emisión</h2><p class="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Los datos fiscales se validan contra los catálogos oficiales MH y luego se copiarán como snapshot al documento DTE.</p></div>
...
<section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    <div><p class="text-xs font-semibold uppercase tracking-[.18em] text-violet-600">Emisor</p><h3 class="mt-2 text-xl font-bold">Identidad fiscal de MEGALOAD</h3><p class="mt-2 text-sm text-slate-500">CAT-009, CAT-012, CAT-013 y CAT-019 se validan también en servidor.</p></div>
    <form method="post" action="<?= route_to('dte_settings.issuer.save') ?>" class="mt-6 grid gap-4 md:grid-cols-2" data-processing-message="Guardando configuración fiscal del emisor…">
        ...
        <label class="md:col-span-2 text-sm font-semibold text-slate-700">Actividad económica · CAT-019
            <select id="issuer_activity_code" name="activity_code" required class="mt-2 w-full" data-placeholder="Buscar actividad económica">
                <option value="">Seleccionar</option>
                <?php foreach($economicActivities as $row): ?><option value="<?= esc($row['code']) ?>" data-description="<?= esc($row['name'], 'attr') ?>" <?= (string)old('activity_code', $issuer['activity_code'] ?? '')===(string)$row['code']?'selected':'' ?>><?= esc($row['code']) ?> · <?= esc($row['name']) ?></option><?php endforeach ?>
            </select>
        </label>
        <div class="md:col-span-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3"><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Descripción CAT-019</p><p id="issuer_activity_description" class="mt-1 text-sm font-semibold text-slate-800"><?= esc($issuer['activity_description'] ?? 'Seleccione una actividad económica.') ?></p></div>

        <label class="md:col-span-2 text-sm font-semibold text-slate-700">Tipo de establecimiento · CAT-009
            <select name="establishment_type_code" class="mt-2 w-full" data-placeholder="Seleccionar tipo de establecimiento"><option value="">Sin asignar</option><?php foreach($establishmentTypes as $row): ?><option value="<?= esc($row['code']) ?>" <?= (string)old('establishment_type_code', $issuer['establishment_type_code'] ?? '')===(string)$row['code']?'selected':'' ?>><?= esc($row['code']) ?> · <?= esc($row['name']) ?></option><?php endforeach ?></select>
        </label>

        <label class="text-sm font-semibold text-slate-700">Departamento · CAT-012
            <select id="issuer_department_code" name="department_code" required data-native="true" class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 font-normal"><option value="">Seleccionar</option><?php foreach($departments as $row): ?><option value="<?= esc($row['code']) ?>" <?= (string)old('department_code', $issuer['department_code'] ?? '')===(string)$row['code']?'selected':'' ?>><?= esc($row['code']) ?> · <?= esc($row['name']) ?></option><?php endforeach ?></select>
        </label>
        <label class="text-sm font-semibold text-slate-700">Municipio · CAT-013
            <select id="issuer_municipality_code" name="municipality_code" required data-native="true" class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 font-normal"><option value="">Seleccionar</option><?php foreach($municipalities as $row): ?><option value="<?= esc($row['code']) ?>" data-department="<?= esc($row['parent_code']) ?>" <?= (string)old('municipality_code', $issuer['municipality_code'] ?? '')===(string)$row['code'] && (string)old('department_code', $issuer['department_code'] ?? '')===(string)$row['parent_code']?'selected':'' ?>><?= esc($row['code']) ?> · <?= esc($row['name']) ?></option><?php endforeach ?></select>
        </label>
...
<section id="emission" class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    <p class="text-xs font-semibold uppercase tracking-[.18em] text-cyan-600">Emisión</p><h3 class="mt-2 text-xl font-bold">Parámetros predeterminados</h3><p class="mt-2 text-sm text-slate-500">Se utilizarán al crear nuevos DTE y se congelarán antes de numerar el documento.</p>
...
<section class="rounded-2xl bg-slate-950 p-6 text-white shadow-xl"><p class="text-xs font-semibold uppercase tracking-[.18em] text-cyan-400">Estado de configuración</p><h3 class="mt-3 text-xl font-bold"><?= $issuer ? 'Emisor configurado' : 'Emisor pendiente' ?></h3><p class="mt-3 text-sm leading-6 text-slate-300"><?= $issuer ? 'Los datos fiscales base están registrados y ahora se validan contra los catálogos MH cargados.' : 'Complete la información del emisor antes de generar el JSON definitivo.' ?></p></section>
<section class="rounded-2xl border border-emerald-200 bg-emerald-50 p-6"><p class="text-xs font-semibold uppercase tracking-[.18em] text-emerald-700">Catálogos conectados</p><div class="mt-4 space-y-2 text-sm text-emerald-900"><p>✓ CAT-009 Tipo de establecimiento</p><p>✓ CAT-012 Departamento</p><p>✓ CAT-013 Municipio</p><p>✓ CAT-019 Actividad económica</p></div></section>
<section class="rounded-2xl border border-amber-200 bg-amber-50 p-6"><p class="text-xs font-semibold uppercase tracking-[.18em] text-amber-700">Seguridad</p><h3 class="mt-2 font-bold text-amber-950">Credenciales fuera de esta tabla</h3><p class="mt-3 text-sm leading-6 text-amber-900">Firma electrónica, token/API y secretos de Hacienda se incorporarán mediante configuración protegida.</p></section>
</aside>
</div>
...
document.addEventListener('DOMContentLoaded', () => {
    const department = document.getElementById('issuer_department_code');
    const municipality = document.getElementById('issuer_municipality_code');
    const activity = document.getElementById('issuer_activity_code');
    const activityDescription = document.getElementById('issuer_activity_description');
    ...
    function refreshMunicipalities() {
        ...
        municipality.innerHTML = '<option value="">Seleccionar</option>';
        municipalityOptions.forEach(option => {
            ...
            if (option.value === current || (!current && option.selected)) node.selected = true;
            municipality.appendChild(node);
        });
        if (municipality.options.length === 1) municipality.value = '';
    }

    function refreshActivityDescription() {
        if (!activity || !activityDescription) return;
        const option = activity.options[activity.selectedIndex];
        activityDescription.textContent = option?.dataset.description || 'Seleccione una actividad económica.';
    }
    ...
    activity?.addEventListener('change', refreshActivityDescription);

    refreshMunicipalities();
    refreshActivityDescription();
});
</script>
<?= $this->endSection() ?>

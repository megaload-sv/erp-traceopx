...
<aside class="w-full bg-slate-950 px-6 py-6 text-white lg:min-h-screen lg:w-72">
    ...
    <nav class="space-y-2">
        ...
        <span class="block rounded-lg px-4 py-3 text-slate-500">Cobros</span>
    </nav>
</aside>
...
<div id="traceopx-processing-overlay" class="fixed inset-0 z-[100] hidden items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" role="status" aria-live="polite" aria-busy="true">
    <div class="w-full max-w-sm rounded-3xl border border-white/10 bg-white p-7 text-center shadow-2xl">
        <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-cyan-50">
            <svg class="h-7 w-7 animate-spin text-cyan-600" viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle class="opacity-20" cx="12" cy="12" r="9" stroke="currentColor" stroke-width="3"></circle><path class="opacity-90" d="M21 12a9 9 0 0 0-9-9" stroke="currentColor" stroke-width="3" stroke-linecap="round"></path></svg>
        </div>
...
window.TraceOpxProcessing={active:false,messageFor(form){if(form.dataset.processingMessage)return form.dataset.processingMessage;const action=(form.getAttribute('action')||'').toLowerCase();if(action.includes('/work-orders/')&&action.endsWith('/issue'))return 'Emitiendo Orden de Trabajo…';if(action.includes('/work-orders/')&&action.endsWith('/start'))return 'Iniciando servicio…';if(action.includes('/mission-log'))return 'Registrando avance operativo…';if(action.includes('/checklist/'))return 'Actualizando checklist operativo…';if(action.includes('/evidence'))return 'Guardando evidencia…';if(action.includes('/billing/'))return 'Procesando facturación…';if(action.includes('/dte-settings/'))return 'Actualizando configuración DTE…';return null;},show(message,form){if(this.active)return false;this.active=true;const overlay=document.getElementById('traceopx-processing-overlay');const label=document.getElementById('traceopx-processing-message');if(label)label.textContent=message||'Procesando solicitud…';if(overlay){overlay.classList.remove('hidden');overlay.classList.add('flex');}document.body.classList.add('overflow-hidden');if(form){form.dataset.processing='true';form.querySelectorAll('button[type="submit"],input[type="submit"]').forEach(button=>{button.disabled=true;button.setAttribute('aria-disabled','true');});}return true;}};
document.addEventListener('submit',event=>{const form=event.target;if(!(form instanceof HTMLFormElement))return;if(form.dataset.processing==='true'||window.TraceOpxProcessing.active){event.preventDefault();event.stopImmediatePropagation();return;}const message=window.TraceOpxProcessing.messageFor(form);if(!message)return;if(!form.checkValidity())return;window.TraceOpxProcessing.show(message,form);},true);
